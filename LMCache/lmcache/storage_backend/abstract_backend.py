...
class LMCBackendInterface(metaclass=abc.ABCMeta):
    ...
    def batched_put(
        ...
        # 倒序存储keys和chunks
        for key, kv_chunk in reversed(list(keys_and_chunks)):
            self.put(key, kv_chunk, blocking=blocking, retrieved_keys=retrieved_keys)
            nchunks += 1
        return nchunks
    ...
    def batched_get(
        ...
        for key in keys:   
            # if not dist.is_initialized() or dist.get_rank() == 0:   
            #     with open('/mnt/baizhuoyan/fair-caching/lmcache-tests/outputs/want_keys.txt', 'a') as f:
            #         f.write(key.to_string() + '\n')
