...
class LMCacheEngine:
    ...
    def retrieve(
        ...
        Union[KVCache, torch.Tensor],
        torch.Tensor,
        List[CacheEngineKey],
        List[Tuple[bool, List[int], int]],
    ]:
        """
        ...
        :return: Tuple[ kv_tensors , ret_mask, retrieved_keys, retrieval_attribution ]
            retrieval_attribution: per retrieved chunk (was_new_client, old_client_ids, token_count)
        """
        num_skip_chunk = 0
        num_skip_tok = 0
        ...
        # 创建key列表以便后续记录
        keys = [self._make_key(chunk_hash, fmt) for chunk_hash in chunk_hashes]
        retrival_iterator = self.engine_.batched_get(keys)

        retrieved_kv_chunks = []
        retrieved_keys = []
        for i, chunk in enumerate(retrival_iterator):
            if chunk is None:
                break
            retrieved_kv_chunks.append(chunk)
            retrieved_keys.append(keys[i])

        # Get attribution info *before* update_after_get (which adds current client to access_map)
        attribution_info = []
        if getattr(self.engine_, "get_attribution_info", None) is not None and len(retrieved_keys) > 0:
            attribution_info = self.engine_.get_attribution_info(retrieved_keys)

        self.engine_.update_after_get(retrieved_keys)
        ...
        if len(retrieved_kv_chunks) == 0:
            ...
            return (), ret_mask, retrieved_keys, []
        ...
        # Build per-chunk attribution with token counts (first chunk may be partial)
        extra_token_len = num_skip_tok - num_skip_chunk * self.chunk_size
        retrieval_attribution = []
        if attribution_info:
            for i, (was_new, old_clients) in enumerate(attribution_info):
                if i == 0:
                    tok = self.chunk_size - extra_token_len
                else:
                    tok = self.chunk_size
                retrieval_attribution.append((was_new, old_clients, tok))

        self.stats_monitor.on_retrieve_finished(monitor_req_id,
                                                retrieved_token_count)
        return ret, ret_mask, retrieved_keys, retrieval_attribution
