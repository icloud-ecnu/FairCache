...
from lmcache.storage_backend.local_backend import client_id_ctx, request_type_ctx
...
def lmcache_store_kv(
    ...
    seq_data_idx = 0
    seq_group_metadata_list = model_input.seq_group_metadata_list
    retrieved_keys_idx = 0
    for seq_group_metadata in seq_group_metadata_list:
        client_id_ctx.set(seq_group_metadata.client_id)
        # 根据 request_id 前缀区分 chat / completion 请求类型
        req_id = getattr(seq_group_metadata, "request_id", None)
        if isinstance(req_id, str) and req_id.startswith("chat-"):
            request_type_ctx.set("chat")
        else:
            request_type_ctx.set("api")
        for seqid, seq_data in seq_group_metadata.seq_data.items():
            status = store_status[seq_data_idx]
            if retrieved_keys_list is not None:
                retrieved_keys = retrieved_keys_list[retrieved_keys_idx]
            else:
                retrieved_keys = []
            ...
            if skip_leading_tokens < seq_len:
                ...
                if len(slot_mapping) > 0:
                    ...
                    engine.store(current_tokens.cpu(), tuple(kv_tuple_list), kv_tensors_mask,
                                skip_existing = True, blocking = False, retrieved_keys = retrieved_keys)
            else:
                stored_token_num = 0
...
def lmcache_retrieve_kv(
...
) -> Tuple["ModelInputForGPUWithSamplingMetadata", bool, Optional[Dict[int, List[float]]], List[Any], Optional[Dict[int, float]]]:
    ...
    Returns credits for dynamic cost attribution (to compensate prefix creators).
    ...
    if engine.config.enable_blending:
        return model_input, False, None, None, None
    ...
    prefill_served = {}
    # Credits to compensate prefix creators when new client shares their block
    credits: Dict[int, float] = {}

    retrieved_keys_list = []

    # For dynamic cost attribution (eta from paper)
    PREFILL_WEIGHT_COST = 0.1
    ETA_ATTRIBUTION = 1.0

    # if not dist.is_initialized() or dist.get_rank() == 0:
    #     with open('/mnt/baizhuoyan/fair-caching/lmcache-tests/outputs/total_seq_len.txt', 'a') as f:
    #         f.write(f"len(seq_group_metadata_list): {len(seq_group_metadata_list)}\n")
    ...
    for seq_group_metadata in seq_group_metadata_list:
        ...
        for seq_id in seq_ids:
            ...
            # if not dist.is_initialized() or dist.get_rank() == 0:
            #     with open('/mnt/baizhuoyan/fair-caching/lmcache-tests/outputs/total_seq_len.txt', 'a') as f:
            #         f.write(f"total_seq_len!!!!!!!!!!!!!!!!!! {total_seq_len}\n")
            ...
            # call lmcache retrieve (returns attribution for dynamic cost)
            retrieve_result = engine.retrieve(full_token_tensor, token_mask)
            if len(retrieve_result) == 4:
                kv_tuple, ret_token_mask, retrieved_keys, retrieval_attribution = retrieve_result
            else:
                kv_tuple, ret_token_mask, retrieved_keys = retrieve_result[:3]
                retrieval_attribution = []

            retrieved_keys_list.append(retrieved_keys)
            ...
            # 如果只是针对chunk一部分的重用，干脆不重用了，只有整个chunk都可以直接复用情况下才考虑
            # 只要有一条请求是这个情况，就直接返回按正常流程做prefill了
            # 我这边也直接hack了，返回None，因为它也没遍历完，我也不好判断，暂时不考虑chunk prefill的情况
            if retrieve_status == RetrieveStatus.CHUNK_PREFILL:
            ...
            # Dynamic cost attribution: if current client hit blocks created by others,
            # adjust hit/miss and issue credits to old clients (was_new_client, old_access_map)
            delta_hit_reduce = 0.0
            if retrieval_attribution:
                for (was_new, old_clients, tok) in retrieval_attribution:
                    if was_new and len(old_clients) > 0:
                        k = len(old_clients)
                        delta_hit_reduce += tok / (k + 1)
                        cost = PREFILL_WEIGHT_COST * tok
                        credit_per_old = ETA_ATTRIBUTION * (cost / k - cost / (k + 1))
                        for old_cid in old_clients:
                            credits[old_cid] = credits.get(old_cid, 0) + credit_per_old

            # 判断请求是否是被preempted之后重新prefill的，只有没有从未被preempt的请求才会被计费
            if seq_group_metadata.sampling_params.preempted is None:
                cliend_id = seq_group_metadata.client_id
                # {client_id:(hit_length, not_hit_length)} — use adjusted hit/miss only when attribution applied
                if delta_hit_reduce > 0:
                    adj_hit = num_computed_tokens - int(round(delta_hit_reduce))
                    adj_miss = total_seq_len - adj_hit
                else:
                    adj_hit = num_computed_tokens
                    adj_miss = total_seq_len - num_computed_tokens
                if cliend_id not in prefill_served:
                    prefill_served[cliend_id] = [adj_hit, adj_miss]
                else:
                    prefill_served[cliend_id][0] += adj_hit
                    prefill_served[cliend_id][1] += adj_miss
    ...
    out_credits = credits if credits else None

    if retrieve_status == RetrieveStatus.CHUNK_PREFILL and \
        num_request_not_found == 0:
        return model_input, True, prefill_served, retrieved_keys_list, out_credits
    ...
    if num_request_not_found < seq_cnt:
        ...
        return rebuilt_model_input, False, prefill_served, retrieved_keys_list, out_credits
    
    logger.debug("Returning the original input!")
    return model_input, False, prefill_served, retrieved_keys_list, out_credits
