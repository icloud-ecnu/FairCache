...
# 不管decode还是prefill都会调用该函数，不过只有prefill才能触发cache检索/取回
...
def new_execute_model(
    ...
    is_skip = False
    prefill_served = None
    retrieved_keys_list = None
    prefill_credits = None
    if retrieve_status != RetrieveStatus.NONE:
        logger.info(f"KV cache retrieving mode: {retrieve_status}")
        model_input, is_skip, prefill_served, retrieved_keys_list, prefill_credits = lmcache_retrieve_kv(
    ...
    if not is_skip:
        ...
        if any([status != StoreStatus.NONE for status in store_status]):
            ...
            lmcache_store_kv(self.model_config, self.parallel_config, model_executable,
                    model_input, self.cache_config, kv_caches, store_status, retrieved_keys_list)
    ...
    output.prefill_served = prefill_served
    output.credits = prefill_credits
