...
class LLMEngine:
    ...
    def step(self) -> List[Union[RequestOutput, EmbeddingRequestOutput]]:
        ...
        if not scheduler_outputs.is_empty():
            ...
            # prefill更新计分板
            if scheduler_outputs.num_prefill_groups > 0 and outputs[0].prefill_served is not None:
                # 输出outputs[0].prefill_served到文件中
                # with open('/mnt/baizhuoyan/fair-caching/lmcache-tests/outputs/total_seq_len.txt', 'a') as f:
                #     f.write("prefill_served: ")
                #     json.dump(outputs[0].prefill_served, f, indent=4)
                #     f.write(',\n')
                self.scheduler[virtual_engine].waiting.update_prefill_served(
                    outputs[0].prefill_served,
                    getattr(outputs[0], "credits", None),
                )
    ...
    def _get_stats(self,
        ...
        num_waiting_each_client = self.scheduler[0].waiting.get_client_queue_lengths()
        ...
        return Stats(
            ...
            num_waiting_each_client=num_waiting_each_client,
            #   KV Cache Usage in %
