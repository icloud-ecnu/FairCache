...
from vllm.core.cpi_req_queue import CPIReqQueue, ReqQueue
...
class Scheduler:
    ...
    def __init__(
        ...
        if self.scheduler_config.fair_computation_enabled:
            self.waiting = CPIReqQueue()  # 使用自定义类
        else:
            self.waiting = ReqQueue()  # 使用原生 deque
    ...
    def _schedule_running(
        ...
        if preempted:
            for seq_group in preempted:
                seq_group.sampling_params.preempted = True
    ...
    def _schedule_prefills(
        ...
        while self._passed_delay(time.time()) and waiting_queue:
            ...
            # print("client_id in seq:", seq_group.client_id)
    ...
    def _schedule_default(self) -> SchedulerOutputs:
        ...
        # Decode process, update CPI corresponding to clients
        # Decode过程，更新clients对应的CPI
        if  num_prefill_groups == 0:
            for scheduled_seq_group in scheduled_seq_groups:
                self.waiting.served[scheduled_seq_group.seq_group.client_id] += self.waiting.decode_weight
