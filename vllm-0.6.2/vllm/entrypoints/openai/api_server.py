...
async def create_chat_completion(request: ChatCompletionRequest,
    ...
    # Extract the client_id and restore the content
    for message in request.messages:
        content = message.get("content", "")
        # Extract the client_id from the content using regular expressions
        match = re.match(r"\[CLIENT_ID:(\d+)\]", content)
        if match:
            # Extract the client_id
            client_id = int(match.group(1))
            
            # Remove the client id from the content and return the original context
